local create = loadstring(game:HttpGet("https://raw.githubusercontent.com/cafehepho300/MaruCrack/main/Theme"))()
    ...
    local Window = create:Win("Hades Hub Premium Script")
...
task.spawn(function()
    ...
    RL.wrapAttackAnimationAsync = function(a, b, c, d, func)
        if not getgenv().Settings.NoAttackAnimation and not _G.NeedAttacking then
            PC.play = shared.cpc
    ...
    while game:GetService("RunService").Stepped:Wait() do
        if #canHits > 0 then
            ...
            if Controller and Controller.equipped and (not game.Players.LocalPlayer.Character.Busy.Value or game.Players.LocalPlayer.PlayerGui.Main.Dialogue.Visible) and game.Players.LocalPlayer.Character.Stun.Value < 1 and Controller.currentWeaponModel then
                if (_G.NeedAttacking or getgenv().Settings.DamageAura) then
                    if getgenv().Settings.NewFastAttack and tick() > AttackCD and not _G.NeedAttacking then
                        resetCD()
                    end
